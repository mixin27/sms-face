...
class AssignToScheduleScrollTable extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.columns = props.columns.concat({
      ...
      render: record => (
        <>
          {record.complain_status == 1 ? (
            <span style={{ color: "green" }}>Accepted</span>
          ) : record.complain_status == 0 ? (
            <span style={{ color: "red" }}>Rejected</span>
          ) : (
            <div>
              <Link
                style={{ color: "#79C908" }}
                to={`/schedules/assign-view/${record.id}/${record.complain_no}`}
              >
                View
              </Link>{" "}
              ...
              <Link
                style={{ color: "#1890FF" }}
                to={`/schedules/assign-accept/${record.id}/${
                  record.complain_no
                }`}
              >
                Accept
  ...
  reject = key => {
    ...
    this.props.rejectComplain(item.id);
    // this.props.deleteData(item.id);
    // message.success('Deleted !');
  };
...
export default AssignToScheduleScrollTable;
